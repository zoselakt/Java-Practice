...
/*
 * 날짜 : 2022. 02. 11.
 * 순서 : 클래스 → 입출력 → 그외(여러가지) → 예외처리 → 배열 → 제네릭
 * 		
 * */
...
//------------------------------------------------------------------------------------
// 객체 : 프로그램의 대상, 생성된 인스턴스의 집합
// 클래스: 객체를 코드로 만든상태 / 속성과 행동를 정의하여 기본적인 데이터를 만든 상태
// 인스턴스: 클래스가 메모리에 생성된 상태
// 멤버변수: 클래스의 세부 속성, 특성

// 메서드: 멤버변수를 이용하여 특정한 동작이나 작업, 클래스의 기능을 구현하는것 / 인스턴스를 생성하지 않고 값을 호출할수 있다.
// 생성자: 객체가 처음 생성될때 호출되어 멤버변수, 인스턴스변수 를 초기화하고 자원을 할당하기도 한다.
// 상속 : extends: 오버라이딩 필요없이 부모값을 사용가능 (class) / 다중상속 x - 한개의 class만 상속
// 		 implements: 반드시 오버라이딩해야한다.(interface) / 다중상속 o - 여러개의 interface상속
//		class와class, interface와interface간에는 extends를 사용하고 class와interface간에는 implements를 사용한다.
// 인터페이스: SOLID를 만족하며 / 동일한 기능을 하기위해 사용 / 다형성

// 매개변수(파라미터): 변수의 특별한 한 종류 / call by value의 형태로 동작, call by reference의 경우 서브루틴 내부에서의동작이 전달인자에 까지 영향을 주게된다.
// 전달인자(argument): 함수를 호출할때 전달되는 실제 값

// 기본변수: 자료형으로 선언된 변수
// 참조변수: 실제 값을 가진 변수가 아니라 값이 들어가 있는 주소를 가지고 있는 변수 / 메모리에 생성된 인스턴스를 가리키는 변수 
// 참조 값: 생성된 인스턴스의 메모리 주소값
//------------------------------------------------------------------------------------
/*
접근권한 지정자
private: 같은 클래스 내에서만 접근이 가능하다. / 상속 불가능
default(package): 현재 패키지에서는 public처럼 사용되고 다른패키지에서는 private처럼 사용된다. 
...
// 기본자료형 : byte, short, int, long, float, double, char, boolean, void
// 래퍼클래스 : Byte, Short, Integer, Long, Float, Double, Character, Boolean, Void

// 기본타입 에서 래퍼클래스 : 박싱(Boxing) / 래퍼클래스에서 기본타입 : 언박싱(UnBoxing)
// 사용 : 매서드의 파라미터값은 객체형태로만 받기 때문에 기본타입의 자료형을 객체(클래스)화 하여 전달한다.
// 장점 : 기본 자료형을 클래스화 하여 클래스의 장점을 활용할 수 있게 된다.
...
// 					예외처리에 사용되는 경우 어떤 종류의 오류인지 출력(getMessage보다 )
...
// 연산결과로 boolean값을 리턴한다. / 참조변수 instanceOf 클래스명(타입)
// 참조변수 a의 타입이 Car타입인지를 체크 / if(a instanceOf Car) 

// getMessage() : 어떤 예외인지 확인해주는 매서드
// getStrackTrace() :

//------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
// 예외의 종류 
// RuntimeException : 주로 프로그래밍 버그나 논리오류에서 발생 / 반드시 예외처리할 필요는 없다.
// ArithmeticException : 수학적오류 / 어떤 수를 0으로 나눌때 발생
// NullPointerException : Null객체를 참조할때 발생
// ClassCastException: 적절치 못하게 클래스를 형 변환 하는 경우
// NegativeArraySizeException: 배열의 크기가 음수값인 경우
// OutOfMemoryException: 사용가능한 메모리가 없는 경우
// NoClassDefFoundException: 원하는 클래스를 찾지 못하였을 경우
// ArrayIndexOutOfBoundsException: 배열을 참조하는 인덱스가 잘못된 경우

// OtherException : 반드시 예외처리를 해야한다.
...
/*
 return값으로 둘 이상의 값을 리턴할때 배열을 만들어 리턴하면 된다.
